...
#include "ExtendibleHashMap/ExtendibleHashMap.h"
...
BOOST_AUTO_TEST_CASE(TestIntegerHashMap)
{
  ...
  }
  {
    ...
    BOOST_TEST_MESSAGE("size/capacity: " << (double)m1.size() / m1.capacity());
  }
...
BOOST_AUTO_TEST_CASE(TestStringHashMap)
{
  constexpr std::size_t elementNum = 1 << 14;
  constexpr std::size_t d = elementNum / 4;
  std::random_device seedGen;
